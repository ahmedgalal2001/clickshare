<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Attribute Value') }}
        </h2>
    </x-slot>

    <section class="mx-2 my-3">
        <form class="max-w-sm mx-auto bg-white shadow p-4"
            action="{{ route('attribute-values.update', $attributeValue->id) }}" method="POST">
            @csrf
            <div class="mb-5">
                <label for="value" class="block mb-2 text-sm font-medium text-gray-900">Value</label>
                <input type="text" id="value" name="value" value="{{ $attributeValue->value }}"
